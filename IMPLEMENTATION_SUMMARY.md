...
6. ✅ REST API for manual notification triggering
7. ✅ Automatic retry mechanism with scheduled tasks
...
**Decision:** Use PostgreSQL with JSONB columns for metadata and template variables

**Rationale:**
- **Flexibility**: No schema changes needed for new metadata fields
- **Queryability**: Can query JSON fields with PostgreSQL JSON operators
- **Performance**: JSONB is binary format, faster than JSON text

**JSONB Columns:**
- `metadata` - Additional notification metadata (documentUrl, documentId, etc.)
- `template_variables` - Template variable values for rendering
